...
function getDishNamesByIds(orderList, ...ids){
      let getDish;
      let selectedDish;
      selectedDish = ids.map((idv)=>{
          getDish=orderList.find((ID)=>ID.id===idv);
          return getDish?.dishName ?? "Unknown Dish";
      }).join(", ");
      return `Selected Dishes: ${selectedDish}`;
}
